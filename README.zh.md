...
从 `0.3.4` 开始，本次请求参数与历史任务候选参数严格分开。用户只说“再生成一个”且上一次同类型任务存在可复用内容时，向导会分别询问是否复用历史 Prompt、参考图/首尾帧和生成参数；任何未确认的历史值都不会合并。本次请求明确给出的值始终优先。用户明确说“再来一个一样的”“其他参数不变”或“不要沿用之前内容”时，可以直接记录对应类别的复用或重置决定，不重复询问。

询问是否增强 Prompt 时，界面会完整展示当前待处理 Prompt，并标明来源为“本次输入”“根据当前请求推断”“向导中填写”或“历史任务复用”。选择增强后仍会继续展示原始 Prompt 与增强后 Prompt 的完整对比；选择保持原文则直接进入模型和参数步骤。

从 `0.3.4` 开始，同一次仍处于“提交状态未知”或“任务运行中”的生成请求会在插件进程内复用同一个幂等操作。网络中断和 HTTP 429 只允许使用原来的 `Idempotency-Key` 恢复；一旦获得 `task_id`，后续只查询该任务，不再重新提交。状态查询遇到临时网络错误、408、425、429 或服务端错误时会退避后继续轮询；进度长时间不变仍视为运行中。前台等待超时会返回任务 ID、最后状态和进度，可通过 `media_task_status` 继续恢复。
...
- 提交恢复状态目前只保存在插件进程内，不写入本地任务日志。重启 DSH/TokensCowork 后，尚未取得 `task_id` 的不确定提交无法自动恢复；已经保留在对话中的任务 ID 仍可手动查询；
